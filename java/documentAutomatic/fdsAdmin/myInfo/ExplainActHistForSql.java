...
public class ExplainActHistForSql {
    public static void main(String[] args) {
        ...
        List<String> resource =
                Arrays.asList(
                        ",,      AAH.REG_DT BETWEEN STR_TO_DATE(CONCAT('20231212', '/00/00/00'), '%Y%m%d/%H/%i/%s') AND STR_TO_DATE(CONCAT('20231214', '/23/59/59'), '%Y%m%d/%H/%i/%s')                                         ",
                        ",,      A.USER_NAME LIKE CONCAT('%', '홍길동', '%')                                               ",
                        ",,      A.LOGIN_ID = '1'                                               ",
                        ",,      AAH.ACT_MENU_CD = 'R'                                          ");
        // 가능한 모든 조합을 생성
        ...
        for (List<String> result : allResult) {
            ...
            System.out.println("EXPLAIN "+resultStr);
        }
