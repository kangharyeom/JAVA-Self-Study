package documentAutomatic.fdsAdmin.doubt.excel;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class DoubtForExcelSqlQuestion {
    public static void main(String[] args) {
        ...
        String tail =
                        "                                                                     "+
                        " ORDER BY PROV_DT ASC                                               ";
        // 예시 데이터
        List<String> resource =
                Arrays.asList(
                        ",,      PROV_DT BETWEEN '?' AND STR_TO_DATE(CONCAT('?', '23:59:59'), '%Y-%m-%d %H:%i:%s')                                                      ",
                        ",,      REG_DT BETWEEN '?' AND STR_TO_DATE(CONCAT('?', '23:59:59'), '%Y-%m-%d %H:%i:%s')                                                       ",
                        ",,      CTN = ?                                                                                                                                ",
                        ",,      CUST_NO = ?                                                                                                                            ",
                        ",,      DOUBT_TYPE = ?                                                                                                                         ",
                        ",,      SYNC_YN = ?                                                                                                                            ",
                        ",,      DOUBT_DETECT_HIST_SEQ = ?                                                                                                              ");
